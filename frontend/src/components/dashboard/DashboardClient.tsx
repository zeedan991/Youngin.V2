...
export default function DashboardClient({
  ...
  const unlockedAchievements = ACHIEVEMENTS.filter((a) => unlockedIds.includes(a.id)).slice(0, 6);
  const lockedAchievements = ACHIEVEMENTS.filter((a) => !unlockedIds.includes(a.id) && !a.hidden).slice(0, 3);
    ...
    <div className="w-full max-w-[1400px] mx-auto min-h-screen">
      ...
      {showDailyXP && (
        <motion.div
          ...
          style={{ background: "var(--dash-accent-dim)", borderColor: "var(--dash-accent)" }}
        >
          <Zap className="w-4 h-4 shrink-0" style={{ color: "var(--dash-accent)" }} />
          <span className="text-sm font-bold" style={{ color: "var(--dash-text)" }}>
            +50 XP — Daily login reward!
            {dailyXPAwarded?.leveledUp && (
              <span className="ml-2 font-black" style={{ color: "var(--dash-accent)" }}>
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 w-full">
            <div>
              ...
              <h1 className="text-2xl md:text-3xl font-extrabold leading-tight tracking-tight mb-1" style={{ color: "var(--dash-text)" }}>
                Welcome back, <span style={{ color: "var(--dash-accent)" }}>{displayName}</span>
              </h1>
          ...
          <div className="mt-6 pt-5 border-t w-full" style={{ borderColor: "var(--dash-border)" }}>
            ...
            <div className="flex items-center gap-4">
              ...
              <p className="text-xs font-extrabold shrink-0" style={{ color: "var(--dash-text)" }}>
                {xpInLevel.toLocaleString()}<span style={{ color: "var(--dash-muted)" }}>/{XP_PER_LEVEL}</span>
              </p>
            </div>
            <p className="text-[10px] mt-1.5" style={{ color: "var(--dash-muted)" }}>
          ...
          {quickActions.map((action, i) => (
            <motion.div key={action.href} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ ...SP, delay: 0.15 + i * 0.05 }}>
              <Link href={action.href}>
                <div className="group rounded-[24px] p-5 border cursor-pointer shadow-xl shadow-slate-200/50 hover:shadow-2xl hover:shadow-[#4F46E5]/10 hover:border-[#4F46E5]/40 transition-all flex items-center gap-4" style={{ background: "var(--dash-surface)", borderColor: "var(--dash-border)" }}>
          ...
          <div className="flex items-center justify-between mb-4 mt-[2px]">
            <div>
              ...
              <p className="text-[11px]" style={{ color: "var(--dash-muted)" }}>
                {unlockedIds.length} of {ACHIEVEMENTS.filter(a => !a.hidden).length} unlocked
              </p>
            </div>
            <Link href="/profile">
              ...
              <div key={achievement.id} className="rounded-xl border p-3 flex items-center gap-3" style={{ background: "rgba(0,0,0,0.02)", borderColor: "var(--dash-border)" }}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <h4 className="text-[12px] font-bold truncate leading-none" style={{ color: "var(--dash-text)" }}>{achievement.title}</h4>
                    <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-full border uppercase tracking-wider shrink-0 ${RARITY_BADGE_COLORS[achievement.rarity]}`}>
                      {RARITY_LABELS[achievement.rarity]}
                    </span>
                  </div>
                  <p className="text-[9px] font-black uppercase tracking-widest leading-none mt-1" style={{ color: "var(--dash-accent)" }}>+{achievement.xpReward} XP</p>
                </div>
              </div>
            ))}

            {/* Locked achievements preview */}
            {lockedAchievements.slice(0, Math.max(0, 4 - unlockedAchievements.length)).map((achievement) => (
              <div key={achievement.id} className="rounded-xl border p-3 flex items-center gap-3 opacity-40" style={{ borderColor: "var(--dash-border)", background: "var(--dash-surface)" }}>
                <div className="h-8 w-8 shrink-0 rounded-[10px] flex items-center justify-center" style={{ background: "rgba(0,0,0,0.05)" }}>
                  <Lock className="w-3.5 h-3.5" style={{ color: "var(--dash-muted)" }} />
                </div>
                <div className="min-w-0 flex-1">
                  <h4 className="text-[12px] font-bold truncate leading-none mb-1" style={{ color: "var(--dash-text)" }}>{achievement.title}</h4>
                  <p className="text-[9px] leading-none" style={{ color: "var(--dash-muted)" }}>Locked — +{achievement.xpReward} XP</p>
                </div>
              </div>
            ))}
          </div>
