...
import { Wand2, Layers, SlidersHorizontal, Download, Loader2, Trash2, Copy, ArrowUp, ArrowDown, Lock } from "lucide-react";
...
export function PanelRight() {
  const { rightPanel, setRightPanel, fabricCanvas, currentGarment, garmentColor, designTitle, setIsSaving, setLastSaved } = useStudioStore();
  ...
  const generateAIGraphic = async () => {
    ...
    try {
      ...
      const img = await FabricImage.fromURL(`data:image/png;base64,${data.imageBase64}`, { crossOrigin: "anonymous" });
  ...
  const exportPDF = useCallback(async () => {
    ...
    const canvasDataURL = fabricCanvas.toDataURL({ format: "png", multiplier: 3 });
    ...
    doc.text(`Print Size: ${currentGarment.printWidth} × ${currentGarment.printHeight}`, 15, 48);
  ...
  const saveDesign = useCallback(async () => {
    ...
    try {
      ...
      const file = new File([blob], `${designTitle}.png`, { type: "image/png" });
  ...
  const deleteSelected = () => {
    ...
    if (active && fabricCanvas) { fabricCanvas.remove(active); fabricCanvas.renderAll(); }
  };
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {rightPanel === "ai" && (
          <div className="space-y-4">
            <div>
              <p className="text-white font-bold text-sm mb-1">✨ AI Graphic Generator</p>
              <p className="text-[#555] text-xs mb-3">Describe a pattern, graphic, or illustration. Gemini AI will generate it directly on your canvas.</p>
              ...
              <div className="space-y-2">
                ...
                >
                  {isGenerating ? <><Loader2 className="w-4 h-4 animate-spin" />Generating…</> : <><Wand2 className="w-4 h-4" />Generate Graphic</>}
                </button>
            ...
            {/* Quick prompts */}
            <div>
              <p className="text-[10px] text-[#555] uppercase tracking-[3px] mb-2 font-bold">Quick Prompts</p>
        ...
        {rightPanel === "layers" && (
          <div className="space-y-2">
            <div className="flex items-center justify-between mb-2">
              <p className="text-white font-bold text-sm">Layers ({getObjects().length})</p>
              <button onClick={deleteSelected} className="text-[#555] hover:text-red-400 transition-colors p-1 rounded">
                <Trash2 className="w-4 h-4" />
              </button>
            </div>
            {getObjects().length === 0 && (
              <p className="text-[#444] text-xs text-center py-8">No objects on canvas yet.<br />Add text, shapes, or images!</p>
            )}
            {[...getObjects()].reverse().map((obj, i) => (
              <div
                key={i}
                onClick={() => { if (fabricCanvas) { fabricCanvas.setActiveObject(obj); fabricCanvas.renderAll(); } }}
              ...
              >
                ...
                >
                  {(obj as any).type === "i-text" || (obj as any).type === "text" ? (
                ...
                <span className="text-[#888] text-xs font-medium capitalize flex-1 truncate">
                  {(obj as any).type === "i-text" ? `"${(obj as any).text?.substring(0, 20)}…"` : (obj as any).type}
                </span>
              </div>
            ))}
          </div>
        ...
        {rightPanel === "filters" && (
          <div className="space-y-4">
            <p className="text-white font-bold text-sm mb-2">Object Adjustments</p>
            <p className="text-[#555] text-xs">Select an object on the canvas to apply adjustments.</p>
              ...
              {[
                ...
                <div key={prop}>
                  <div className="flex justify-between text-xs text-[#555] mb-1">
                    <span>{label}</span>
                    <span>{Math.round(((fabricCanvas?.getActiveObject() as any)?.[prop] ?? 1) * (prop === "opacity" ? 100 : 1))}
                    {prop === "opacity" ? "%" : "°"}</span>
                  </div>
                  <input
                    type="range" min={min} max={max} step={step}
                    value={(fabricCanvas?.getActiveObject() as any)?.[prop] ?? (prop === "opacity" ? 1 : 0)}
                    onChange={(e) => {
        ...
        {rightPanel === "export" && (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-[#555] text-xs mb-4">All exports are production-ready for tailors and factories.</p>
            </div>

            <div className="bg-[#1A1A1A] rounded-2xl border border-[#2A2A2A] p-4 space-y-2">
              <p className="text-xs font-bold text-[#888] uppercase tracking-wider">Print Specifications</p>
              <div className="space-y-1 text-xs">
                <div className="flex justify-between"><span className="text-[#555]">Garment</span><span className="text-white font-medium">{currentGarment.label}</span></div>
                <div className="flex justify-between"><span className="text-[#555]">Print Size</span><span className="text-white font-medium">{currentGarment.printWidth} × {currentGarment.printHeight}</span></div>
                <div className="flex justify-between"><span className="text-[#555]">Print Zone</span><span className="text-white font-medium capitalize">{currentGarment.printZone}</span></div>
                <div className="flex justify-between"><span className="text-[#555]">Export DPI</span><span className="text-[#00E5FF] font-bold">300 DPI</span></div>
                <div className="flex justify-between"><span className="text-[#555]">Format</span><span className="text-white font-medium">PNG (transparent)</span></div>
              </div>
            </div>
            ...
            <p className="text-center text-[10px] text-[#444]">Designs saved here appear in your profile history and can be sent to tailors or our partner factories.</p>
          </div>
        )}
      </div>
    </div>
