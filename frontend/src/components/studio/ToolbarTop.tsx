"use client";
import { useStudioStore, GARMENTS, type ActiveTool } from "@/store/useStudioStore";
import { cn } from "@/lib/utils";
import {
  Undo2, Redo2, ChevronDown, Download, Trash2
} from "lucide-react";
import { useState } from "react";

export function ToolbarTop() {
  const {
    currentGarment, setGarment,
    designTitle, setDesignTitle,
    undo, redo,
    isSaving, lastSaved,
    fabricCanvas,
    setRightPanel
  ...
  const clearCanvas = () => {
    if (fabricCanvas) {
      fabricCanvas.clear();
      fabricCanvas.set("backgroundColor", useStudioStore.getState().garmentColor);
  ...
  return (
    <div className="h-12 flex items-center gap-3 px-4 bg-[#0F0F0F] border-b border-[#1E1E1E] z-50 shrink-0">
      ...
      <div className="flex items-center gap-2 mr-2 pr-4 border-r border-[#1E1E1E]">
        <span className="text-white font-black text-sm tracking-[4px]">YOUNGIN</span>
        <span className="text-[#00E5FF] text-[10px] font-bold px-1.5 py-0.5 rounded bg-[#00E5FF]/10">CAD</span>
      </div>
      ...
      <div className="relative mr-2">
        ...
        >
          ...
          <ChevronDown className={cn("w-3 h-3 text-[#555] transition-transform", garmentOpen && "rotate-180")} />
        </button>
        ...
        {garmentOpen && (
          <div className="absolute top-full left-0 mt-1 w-56 bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl shadow-2xl z-50 py-1 overflow-hidden">
            {GARMENTS.map((g) => (
              <button
                key={g.id}
                onClick={() => { setGarment(g); setGarmentOpen(false); }}
                className={cn(
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[9px] text-[#555] uppercase tracking-wider">{g.printWidth} × {g.printHeight}</p>
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="flex items-center gap-1">
        <button onClick={undo} title="Undo" className="p-1.5 rounded-md text-[#666] hover:text-white hover:bg-[#1A1A1A] transition-all">
          <Undo2 className="w-4 h-4" />
        </button>
        <button onClick={redo} title="Redo" className="p-1.5 rounded-md text-[#666] hover:text-white hover:bg-[#1A1A1A] transition-all">
          <Redo2 className="w-4 h-4" />
        </button>
        <div className="w-px h-4 bg-[#222] mx-1" />
        <button onClick={clearCanvas} title="Clear Canvas" className="flex items-center gap-1.5 px-2 py-1.5 rounded-md text-red-500/80 hover:text-red-400 hover:bg-red-500/10 transition-all text-xs font-bold">
