...
import {
  getTailorOrders,
  getTailorTransactions,
  getTailorDesigns,
} from "@/app/(tailor)/tailor/actions";
...
export default function TailorDashboardPage() {
  ...
  const STATS = [
    ...
    },
    {
      ...
      value: `$${escrow}`,
      icon: Clock,
      change: `${pendingOrders + inProgressOrders} orders`,
    ...
    <div className="p-6 lg:p-10 space-y-8 min-h-screen bg-slate-50">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {STATS.map((s) => (
          <div
            key={s.label}
            className={`relative rounded-2xl p-5 border border-slate-200 overflow-hidden bg-white shadow-xl shadow-slate-200/50`}
          >
            <div
              className={`absolute inset-0 bg-gradient-to-br opacity-50 ${s.color}`}
            />
            <div className="relative z-10">
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center mb-3 bg-white shadow-sm border border-slate-100`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 rounded-2xl border border-slate-200 overflow-hidden bg-white shadow-xl shadow-slate-200/50">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50">
            ...
            <Link
              href="/tailor/orders"
              className="text-[10px] font-black text-slate-400 hover:text-[#4F46E5] flex items-center gap-1 uppercase tracking-wider transition-colors"
            >
            ...
            {orders.slice(0, 4).map((o) => (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <span
                    className={`inline-flex items-center text-[9px] font-black px-2 py-0.5 rounded-full border uppercase tracking-wider mt-1 shadow-sm ${statusColor[o.status]}`}
                  >
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="rounded-2xl border border-slate-200 p-5 bg-white shadow-xl shadow-slate-200/50">
            <h3 className="font-black text-slate-900 text-xs uppercase tracking-widest mb-4 flex items-center gap-2">
              <Users className="w-3.5 h-3.5 text-[#4F46E5]" /> Business Stats
            </h3>
            ...
            <div className="space-y-2">
              {[
                ...
                <Link
                  key={a.href}
                  href={a.href}
                  className="flex items-center justify-between px-3 py-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-slate-300 hover:bg-slate-100 transition-all group shadow-sm"
                >
                  <div className="flex items-center gap-3">
                    <a.icon className="w-3.5 h-3.5 text-[#4F46E5]" />
                    <span className="text-slate-600 group-hover:text-slate-900 text-xs font-bold transition-colors">
                      {a.label}
                    </span>
                  </div>
