  ...
  return (
    <div className="p-6 lg:p-10 space-y-6 min-h-screen bg-slate-50">
      <div>
        ...
        <h1 className="text-3xl font-black text-slate-900" style={{ fontFamily: "var(--font-syne), sans-serif" }}>Order Pipeline</h1>
        <p className="text-slate-500 text-sm mt-1">Manage every client request from quote to delivery.</p>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 overflow-hidden bg-white shadow-xl shadow-slate-200/50">
        ...
        {loading ? (
          ...
          <div className="divide-y divide-slate-100">
            {filtered.map((o) => (
              <div key={o.id} className="grid grid-cols-[2fr_2fr_1fr_1fr_1fr] gap-4 items-center px-6 py-4 hover:bg-slate-50 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-slate-900 font-bold text-sm truncate">{o.client_name}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Scissors className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                  <span className="text-slate-600 text-sm truncate">{o.garment}</span>
                </div>
                <span className="text-slate-500 text-xs font-bold">{o.deadline}</span>
                <div className="flex items-center gap-1">
                  <DollarSign className="w-3 h-3 text-emerald-500" />
                  <span className="text-slate-900 font-black text-sm">{o.amount}</span>
                </div>
                <span className={`inline-flex items-center text-[9px] font-black px-2.5 py-1 rounded-full border uppercase tracking-wider w-fit shadow-sm ${statusColor[o.status]}`}>
