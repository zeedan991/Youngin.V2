    ...
    <div className="p-6 lg:p-10 space-y-6 min-h-screen bg-slate-50">
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-900" style={{ fontFamily: "var(--font-syne), sans-serif" }}>Portfolio</h1>
          <p className="text-slate-500 text-sm mt-1">Showcase your craft to potential clients.</p>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="aspect-square rounded-2xl bg-white shadow-xl shadow-slate-200/50 border border-slate-200 animate-pulse" />
          ))}
        </div>
      ) : designs.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-slate-900 font-black text-lg mb-2">No designs yet</h3>
          <p className="text-slate-500 text-sm max-w-sm mb-6">Create your first garment in the 2D Studio and it will appear here automatically.</p>
          <Link href="/studio" className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-black uppercase tracking-widest text-white shadow-lg shadow-indigo-600/20" style={{ background: "linear-gradient(135deg, #4F46E5, #3730A3)" }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {designs.map((d) => (
            <div key={d.id} className="group relative rounded-2xl overflow-hidden border border-slate-200 hover:border-[#4F46E5]/40 transition-all cursor-pointer bg-white shadow-xl shadow-slate-200/50">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-4">
                <p className="text-white font-black text-sm truncate">{d.title || "Untitled"}</p>
                <p className="text-white/70 text-[10px] uppercase tracking-wider">{d.type || "Garment"}</p>
              </div>
              <div className="p-4 border-t border-slate-100">
                <p className="text-slate-900 font-bold text-xs truncate">{d.title || "Untitled Collection"}</p>
                <p className="text-slate-400 text-[10px] uppercase tracking-wider mt-0.5">{d.type || "tshirt"}</p>
              </div>
            </div>
          ))}
        </div>
