...
export default function TailorFinancialsPage() {
  ...
  const totalEarnings = transactions
    .filter((t) => t.amount > 0 && t.status !== "payout")
  ...
  const STATS = [
    ...
    {
      label: "In Escrow",
      value: `$${escrow.toLocaleString()}.00`,
      icon: Clock,
      change: `${transactions.filter((t) => t.status === "escrow").length} orders`,
      iconColor: "text-yellow-500",
    },
    {
      label: "Total Earnings",
      value: `$${totalEarnings.toLocaleString()}.00`,
      icon: TrendingUp,
      change: "+8.2%",
    ...
    <div className="p-6 lg:p-10 space-y-6 min-h-screen bg-slate-50">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">
          Track your earnings, escrow balances, and payouts.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {STATS.map((s) => (
          <div
            key={s.label}
            className="rounded-2xl p-5 border border-slate-200 bg-white shadow-xl shadow-slate-200/50"
          >
            <div className="flex items-start justify-between mb-4">
              <div
                className={`w-8 h-8 rounded-lg bg-slate-50 border border-slate-100 shadow-sm flex items-center justify-center ${s.iconColor}`}
              >
      ...
      <div className="rounded-2xl border border-slate-200 overflow-hidden bg-white shadow-xl shadow-slate-200/50">
        ...
        <div className="divide-y divide-slate-100">
          {filtered.map((t) => (
            <div
              key={t.id}
              className="grid grid-cols-[1.5fr_2fr_2fr_1.5fr_1fr_1fr] gap-4 items-center px-6 py-4 hover:bg-slate-50 transition-colors"
            >
              ...
              <span
                className={`font-black text-sm ${t.amount > 0 ? "text-green-500" : "text-red-500"}`}
              >
                {t.amount > 0 ? "+" : ""}${Math.abs(t.amount).toLocaleString()}
                .00
              </span>
              <span
                className={`inline-flex items-center gap-1 text-[9px] font-black px-2.5 py-1 rounded-full border uppercase tracking-wider w-fit shadow-sm ${statusBadge[t.status]}`}
              >
                {t.status === "payout" && (
