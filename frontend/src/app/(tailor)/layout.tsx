...
export default async function TailorLayout({ children }: { children: ReactNode }) {
  const profileRes = await fetchLiveProfile();
  let initialProfile = undefined;
  
  if (profileRes.success && profileRes.data) {
    initialProfile = {
      full_name: (profileRes.data as any).username || (profileRes.data as any).full_name || "Tailor",
