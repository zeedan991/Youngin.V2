      ...
      <div className="relative overflow-hidden rounded-3xl shadow-2xl border border-slate-800">
        <div className="flex items-center justify-between px-6 py-4 bg-[#0f0f14] border-b border-slate-800">
          <div className="flex items-center gap-3">
            <motion.span animate={{ scale: [1, 1.4, 1], opacity: [1, 0.5, 1] }} transition={{ repeat: Infinity, duration: 1.4 }} className="h-2.5 w-2.5 rounded-full bg-[#4F46E5]" />
            <span className="text-xs font-bold tracking-[3px] uppercase text-slate-400">Live Body Scan · AI Active</span>
          </div>
          <div className="flex items-center gap-3">
            {inView && (<motion.span initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 2.8 }} className="rounded-full bg-[#4F46E5]/15 px-3 py-1 text-[11px] font-extrabold text-[#4F46E5]">33 / 33 Landmarks ✓</motion.span>)}
        ...
        <div className="grid md:grid-cols-[1fr_1fr] min-h-[300px]">
          <div className="relative flex items-center justify-center py-6 bg-[#0f0f14] border-r border-slate-800">
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none"><div className="w-24 h-64 rounded-full bg-[#4F46E5]/6 blur-[60px]" /></div>
            <svg viewBox="0 0 200 420" className="relative z-10 h-[220px] w-auto overflow-visible" fill="none">
              <defs>
                ...
                <linearGradient id="scan-sweep" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="transparent"/><stop offset="35%" stopColor="#4F46E5" stopOpacity="0.9"/><stop offset="65%" stopColor="#4F46E5" stopOpacity="0.9"/><stop offset="100%" stopColor="transparent"/></linearGradient>
              </defs>
              ...
              {LM.map(([cx, cy], i) => (
                <motion.circle key={i} cx={cx} cy={cy} r={i < 11 ? 2.5 : 3.8} fill={i < 11 ? "rgba(255,180,210,0.9)" : "#4F46E5"} filter="url(#lm-glow)" initial={{ scale: 0, opacity: 0 }} animate={inView ? { scale: 1, opacity: 1 } : {}} transition={{ delay: 1.3 + i * 0.06, type: "spring", stiffness: 300, damping: 12 }} />
              ))}
              {inView && (<motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.6 }}><line x1="62" y1="54" x2="138" y2="54" stroke="rgba(255,77,148,0.7)" strokeWidth="1" strokeDasharray="2,2"/><line x1="62" y1="50" x2="62" y2="58" stroke="rgba(255,77,148,0.9)" strokeWidth="1.5"/><line x1="138" y1="50" x2="138" y2="58" stroke="rgba(255,77,148,0.9)" strokeWidth="1.5"/><rect x="80" y="44" width="40" height="14" rx="4" fill="rgba(255,77,148,0.2)"/><text x="100" y="54.5" textAnchor="middle" fill="#4F46E5" fontSize="7.5" fontWeight="bold">44.7 cm</text></motion.g>)}
              {inView && (<motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.9 }}><line x1="10" y1="152" x2="51" y2="152" stroke="rgba(255,77,148,0.5)" strokeWidth="1" strokeDasharray="2,2"/><rect x="-16" y="144" width="26" height="14" rx="4" fill="rgba(255,77,148,0.2)"/><text x="-3" y="154" textAnchor="middle" fill="#4F46E5" fontSize="6.5" fontWeight="bold">78.1</text></motion.g>)}
              {inView && (<motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3.1 }}><line x1="150" y1="212" x2="150" y2="382" stroke="rgba(255,77,148,0.4)" strokeWidth="1" strokeDasharray="2,2"/><line x1="146" y1="212" x2="154" y2="212" stroke="rgba(255,77,148,0.7)" strokeWidth="1.5"/><line x1="146" y1="382" x2="154" y2="382" stroke="rgba(255,77,148,0.7)" strokeWidth="1.5"/><rect x="152" y="282" width="34" height="14" rx="4" fill="rgba(255,77,148,0.2)"/><text x="169" y="292" textAnchor="middle" fill="#4F46E5" fontSize="6.5" fontWeight="bold">82.4 cm</text></motion.g>)}
          ...
          <div className="flex flex-col bg-white">
                ...
                <motion.div key={m.key} className="flex items-center gap-4 px-6 py-4 hover:bg-slate-50 transition-colors" initial={{ opacity: 0, x: 24 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ ...SP, delay: 1.5 + i * 0.15 }}>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1.5 h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                      <motion.div className="h-full bg-gradient-to-r from-[#4F46E5] to-[#ff79b0] rounded-full" initial={{ width: 0 }} animate={inView ? { width: `${m.pct}%` } : {}} transition={{ delay: 2.0 + i * 0.15, duration: 1.2, ease: "easeOut" }} />
                    </div>
                  </div>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 3.2, ...SP }} className="m-5 rounded-2xl bg-gradient-to-br from-[#4F46E5] to-[#e0307a] p-5 text-white shadow-lg">
    ...
    <section className="relative z-10 overflow-hidden bg-transparent py-28 sm:py-40 border-t border-slate-200">
      ...
      <div className="relative mx-auto max-w-7xl px-6 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={SP} viewport={{ once: true }}>
          <p className="text-[10px] font-bold uppercase tracking-[4px] text-[#4F46E5] mb-8">The New Standard</p>
          <h2 className="font-display text-4xl sm:text-6xl md:text-7xl font-black text-slate-900 leading-[1.05] tracking-tight">
            Size&nbsp;S, M, L, XL<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#4F46E5] to-violet-400">were&nbsp;a&nbsp;lie.</span>
          </h2>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, ...SP }} viewport={{ once: true }} className="mt-12 flex flex-wrap items-center justify-center gap-4">
            <Link href="/login">
              <motion.button whileHover={{ scale: 1.05, boxShadow: "0 12px 32px rgba(255,77,148,0.35)" }} whileTap={{ scale: 0.97 }} transition={SP_FAST} className="rounded-full bg-[#4F46E5] px-10 py-4 text-sm font-bold text-white hover:bg-[#ff3382] transition-colors">
                Start Your Scan →
              </motion.button>
            </Link>
      ...
      <div className="fixed top-5 left-0 right-0 z-50 px-4 sm:px-8 flex justify-center pointer-events-none">
        <motion.nav initial={{ opacity: 0, y: -24 }} animate={{ opacity: 1, y: 0 }} transition={{ ...SP, delay: 0.05 }}
          ...
          <div className="hidden items-center gap-8 md:flex">
            {navLinks.map((link, i) => (
              <motion.a key={link} href="#" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ ...SP, delay: 0.1 + i * 0.05 }}
                className="text-[13px] font-semibold tracking-wide text-white/80 transition-colors hover:text-[#4F46E5]">{link}</motion.a>
            ))}
          </div>
          ...
          <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ ...SP, delay: 0.3 }} className="hidden items-center gap-5 md:flex">
            <Link href="/login" className="text-[13px] font-semibold tracking-wide text-white/80 hover:text-[#4F46E5] transition-colors">Sign In</Link>
            <Link href="/login">
        ...
        <div className="flex-1 text-left">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.1 }}
            ...
            {["±1mm precision", "33 landmarks", "2.3s scan"].map((t, i) => (
              <span key={i} className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-[#4F46E5]/60" />{t}</span>
            ))}
          </motion.div>
        </div>
          ...
          <div className="absolute right-0 top-1/2 -translate-y-1/2 flex flex-col gap-3 z-30 hidden xl:flex">
            {[{ name: "Shoulder", val: "44.7 cm" }, { name: "Inseam", val: "82.4 cm" }].map((item, i) => (
              <motion.div key={item.name} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.4 + i * 0.1 }}
                ...
                <p className="text-[9px] font-bold text-[#4F46E5] uppercase tracking-wider mb-1">{item.name}</p>
            ...
            <div className="order-1 lg:order-2">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-4 py-1.5 text-[9px] font-bold uppercase tracking-[3px] text-slate-500 mb-5 shadow-sm">
                  <span className="h-1.5 w-1.5 rounded-full bg-[#4F46E5]" /> Frictionless Sizing
                </span>
                <h2 className="font-display text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight leading-[1.05] text-slate-900 mb-5">
                  Ditch the tape.<br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#4F46E5] to-[#ff79b0]">Know your exact fit.</span>
                </h2>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98, y: 20 }} whileInView={{ opacity: 1, scale: 1, y: 0 }} transition={SP} viewport={{ once: true }}
            ...
            <div className="max-w-2xl relative z-10 flex flex-col items-start text-left">
              <p className="text-[11px] font-bold uppercase tracking-[4px] text-slate-400 mb-6 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-[#4F46E5] animate-pulse" /> Are you a tailor?
              </p>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-12 md:flex-row md:items-start md:justify-between mb-16">
            <div className="flex flex-col gap-4 max-w-xs">
              ...
              {subscribed ? (
                <p className="mt-2 text-sm font-bold text-[#4F46E5]">✓ You&apos;re on the list!</p>
              ) : (
                <form onSubmit={handleSubscribe} className="flex bg-white/5 rounded-full border border-white/10 mt-2 overflow-hidden max-w-xs" aria-label="Email subscription form">
                  ...
                  <button type="submit" className="px-5 bg-white text-black font-bold text-xs hover:bg-[#4F46E5] hover:text-white transition-all">Join</button>
                </form>
              )}
            </div>
              ...
              {[
                ...
                <div key={heading}>
                  ...
                  <ul className="space-y-3 font-medium text-white/40">
                    {links.map((l) => (<li key={l}><a href="#" aria-label={l} className="hover:text-[#4F46E5] transition-colors text-sm">{l}</a></li>))}
                  </ul>
                </div>
              ))}
            </div>
