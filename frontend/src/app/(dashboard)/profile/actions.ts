  ...
  if (profileData) {
    ...
    return {
      ...
      data: {
        ...
        designs_count:
          designsRes.data?.length || profileData.designs_count || 0,
      },
    };
  }
...
export async function updateProfile(formData: FormData) {
  ...
  if (instagram !== null && instagram !== undefined)
    updatePayload.instagram = instagram || null;
  ...
  if (tailorPriceFrom)
    updatePayload.tailor_price_from = parseFloat(tailorPriceFrom);
...
export async function awardDailyLoginXP(): Promise<{
  ...
  try {
    ...
    if (fetchErr) {
      ...
      console.warn(
        "Daily XP fetch failed (column may not exist yet):",
        fetchErr.message,
      );
    ...
    if (updateErr) {
      ...
      const { error: retryErr } = await supabase
        ...
        .eq("id", user.id);
      if (retryErr)
        return { success: false, awarded: false, error: retryErr.message };
    }
...
/**
 * ─── DELETE PROFILE ────────────────────────────────────────────
 * Completely drops the user's Row Level Security data and calls the goTrue server api to drop the authentication record.
 */
export async function deleteProfile() {
  try {
    ...
    // Since profiles has `on delete cascade` referencing `auth.users`, we can theoretically just call admin.deleteUser()
    // However, on standard Supabase clients, deleting the user from auth.users requires service role key.
    // Instead, we will delete their profile from the public schema and also use supabase.rpc or a soft delete if delete is prohibited.
    
    // Attempt standard deletion from profiles table
    const { error: profileErr } = await supabase.from('profiles').delete().eq('id', user.id);
    if (profileErr) return { success: false, error: profileErr.message };

    // Try to delete auth record (might fail depending on RLS/Settings, but we already wiped the profile)
    ...
    return { success: false, error: err.message || "Failed to delete completely." };
  }
}
