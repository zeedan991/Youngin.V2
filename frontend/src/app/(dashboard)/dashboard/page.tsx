    ...
    <div className="w-full">
      <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={SP}>
          ...
          <p className="text-slate-400 text-lg">Here is your style command center, {user.name.split(" ")[0]}.</p>
        </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        <motion.div 
          ...
          className="md:col-span-2 lg:col-span-2 rounded-3xl border border-white/10 bg-[#111116] p-8 relative overflow-hidden group"
        >
          ...
          <div className="flex items-center gap-6 mb-8 relative z-10">
            <div className="h-20 w-20 shrink-0 rounded-2xl bg-gradient-to-br from-[#FF4D94] to-[#B8005C] shadow-inner flex items-center justify-center border-4 border-black">
              <span className="text-black font-extrabold text-2xl tracking-tighter">LV.{user.level}</span>
            </div>
            <div>
              <h2 className="text-2xl font-bold tracking-tight mb-1">{user.name}</h2>
              <div className="flex items-center gap-2 text-slate-400">
                <Sparkles className="w-4 h-4 text-yellow-400" />
                <span className="font-medium text-sm text-yellow-400">{user.title}</span> &bull; 
                <span className="text-sm border border-white/10 px-2 py-0.5 rounded-md flex items-center gap-1">
          ...
          <div className="relative z-10 w-full bg-black/40 rounded-full h-3 border border-white/5 overflow-hidden">
            <motion.div 
        ...
        <motion.div 
          ...
          className="md:col-span-1 lg:col-span-1 rounded-3xl border border-white/10 bg-[#111116] p-8 flex flex-col justify-between"
        >
          <div>
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-bold text-slate-300">Latest Scan</h3>
        ...
        <motion.div 
          ...
          className="md:col-span-3 lg:col-span-1 rounded-3xl border border-white/10 bg-[#111116] p-8 flex flex-col"
        >
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-bold text-slate-300">Active Pipelines</h3>
            <TrendingUp className="w-5 h-5 text-slate-400" />
          </div>
          <div className="flex-1 flex flex-col gap-4">
            {projects.map((proj, i) => (
              <div key={i} className="flex-1 rounded-2xl bg-white/[0.03] border border-white/5 p-4 flex flex-col justify-between group hover:bg-white/[0.05] transition-colors cursor-pointer">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-semibold text-sm leading-tight text-white mb-1 group-hover:text-[#FF4D94] transition-colors">{proj.title}</h4>
                    <p className="text-xs text-slate-500">{proj.meta}</p>
                  </div>
                </div>
                <div className="mt-4 flex items-center gap-2">
                  <div className="flex-1 h-1 bg-white/5 rounded-full overflow-hidden">
                    <div className="h-full bg-white/40 rounded-full w-[60%]" />
                  </div>
                  <span className="text-[10px] font-bold tracking-wider uppercase text-slate-400">{proj.status}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <motion.div 
          ...
          className="md:col-span-3 lg:col-span-4 rounded-3xl border border-white/10 bg-[#111116] p-8"
        >
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
             ...
             <button className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1 hover:text-[#FF4D94] transition-colors">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {badges.map((badge, i) => {
              ...
              return (
                <div key={i} className="rounded-2xl bg-black/40 border border-white/5 p-5 flex items-center gap-4 hover:border-white/20 transition-all cursor-default">
                  <div className={`h-12 w-12 rounded-xl bg-white/5 flex items-center justify-center shrink-0 border border-white/10 shadow-inner`}>
