...
const SCAN_API =
  process.env.NEXT_PUBLIC_SCAN_API_URL || "http://localhost:5001";
const accent = "#4F46E5";
...
const SCAN_PHASES = [
  ...
  "Mapping silhouette contours...",
  "Building 3D mesh model...",
...
function PhotoBox({
  ...
  return (
    <div className="flex flex-col gap-2 items-center w-full h-full justify-center">
      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 drop-shadow-sm">
        {label}
      </span>
      <button
        type="button"
        ...
        {preview ? (
          <>
            ...
            <div className="absolute inset-0 bg-white/70 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
              <span className="text-slate-900 text-xs font-bold tracking-wider">
                CHANGE
              </span>
            </div>
          ...
          <div className="flex flex-col items-center justify-center gap-4 h-full px-5">
            <div
              className="w-14 h-14 rounded-[18px] flex items-center justify-center transition-transform group-hover:scale-110 group-hover:-translate-y-1 shadow-sm"
              style={{
            ...
            <div className="text-center">
              <p className="text-slate-800 text-sm font-black tracking-wide mb-1.5">
                Tap to Upload
              </p>
              <p className="text-slate-500 text-[10px] leading-snug px-1">
                {hint}
              </p>
            </div>
          </div>
    ...
    <div className="flex justify-center items-center gap-1 mb-8 pt-2">
      {steps.map((s, i) => (
        <div key={s} className="flex items-center">
          <div
            className="flex items-center gap-2 px-4 py-1.5 rounded-full text-[11px] font-bold tracking-widest uppercase transition-all duration-500"
            style={{
              background:
                i === current
                  ? accentGlow
                  : i < current
                    ...
                    : "transparent",
              color:
                i < current ? "#16a34a" : i === current ? accent : "#94a3b8",
          ...
          >
            {i < current ? (
              ...
              <span
                ...
                style={{
                  background: i === current ? accent : "#cbd5e1",
                  color: i === current ? "#fff" : "white",
                }}
              >
                {i + 1}
              </span>
          ...
          {i < steps.length - 1 && (
            <div
              ...
              style={{
                background: i < current ? "rgba(74,222,128,0.5)" : "#e2e8f0",
              }}
            />
          )}
        </div>
...
export default function AISizingPage() {
  const [step, setStep] = useState<"guide" | "upload" | "scanning" | "results">(
    "guide",
  );
  const [frontFile, setFrontFile] = useState<File | null>(null);
  const [sideFile, setSideFile] = useState<File | null>(null);
  const [height, setHeight] = useState(170);
  const [gender, setGender] = useState<"neutral" | "male" | "female">(
    "neutral",
  );
  ...
  const downloadMeasurements = () => {
    ...
    const lines = Object.entries(result.measurements)
      .map(
        ([k, v]: any) =>
          `${MEASUREMENT_META[k]?.label ?? k}: ${Number(v).toFixed(1)} cm`,
      )
      .join("\n");
    ...
    const a = Object.assign(document.createElement("a"), {
      ...
      download: "youngin_measurements.txt",
    });
    a.click();
  };
          ...
          {step === "guide" && (
            ...
            >
              ...
              <div className="flex flex-col gap-6">
                <div>
                  <h1
                    className="text-4xl lg:text-5xl font-black tracking-tight leading-tight mb-2 text-slate-900"
                    style={{ fontFamily: "var(--font-syne)" }}
                  >
                    AI Body <span style={{ color: accent }}>Scan</span>
                  </h1>
                  <p className="text-slate-500 text-sm lg:text-base leading-relaxed max-w-lg">
                    Upload your front and side profile. Get 11 exact,
                    tailor-ready measurements and a fully interactive 3D mesh
                    model of your body in exactly 60 seconds.
                  </p>
                </div>
                  ...
                  {[
                    {
                      ...
                      icon: "👕",
                      title: "Tight Fit",
                      desc: "Wear fitted clothing so contours are highly visible.",
                    },
                    {
                      n: "02",
                      icon: "💡",
                      title: "Bright Light",
                      desc: "Stand facing the light source against a totally plain background.",
                    },
                    {
                      n: "03",
                      icon: "🧍",
                      title: "A-Pose",
                      desc: "Stand 2 metres away. Arms slightly away from waist.",
                    },
                  ].map((card, i) => (
                    <motion.div
                      key={card.n}
                    ...
                    >
                      ...
                      <p className="text-slate-900 text-sm font-bold mb-1">
                        {card.title}
                      </p>
                ...
                <div className="relative rounded-3xl p-5 border bg-white shadow-xl shadow-slate-200/50 border-green-200 overflow-hidden">
                  ...
                  <div className="flex items-start gap-4 relative z-10">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] py-0.5 text-slate-500 leading-relaxed font-semibold">
                        Wear figure-hugging clothes. Stand perfectly straight
                        with arms slightly out.
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-start gap-4 relative z-10">
                    <div className="w-12 h-16 rounded border-2 border-red-400 flex items-center justify-center bg-red-50 shrink-0">
                      ...
                      >
                        ...
                        <path
                          d="M6 10c-2 2-1 8 0 10c3 1 6 1 8 0c1-2 2-8 0-10C12 9 8 9 6 10z"
                    ...
                    <div>
                      ...
                      <p className="text-[10px] py-0.5 text-slate-500 leading-relaxed font-semibold">
                        Avoid baggy or loose clothing. Do not cross arms or
                        stand at an angle.
                      </p>
                    </div>
                  </div>
            ...
            >
              <div className="text-center mb-10">
                ...
                <p className="text-slate-500 text-sm max-w-md mx-auto leading-relaxed">
                  Provide clear, well-lit photos. The AI utilizes these to
                  extract your exact dimensions.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-[800px] items-stretch">
                ...
                <div className="flex flex-col h-full bg-white border border-slate-200 shadow-xl shadow-slate-200/50 rounded-3xl p-5">
                  <PhotoBox
                    label="Front View"
                    hint="Stand straight, arms slightly out"
                    file={frontFile}
                ...
                <div className="flex flex-col h-full bg-white border border-slate-200 shadow-xl shadow-slate-200/50 rounded-3xl p-5">
                  <PhotoBox
                    label="Side View"
                    hint="Turn 90°, look perfectly straight"
                    file={sideFile}
                ...
                <div className="flex flex-col justify-between h-full bg-white border border-slate-200 shadow-xl shadow-slate-200/50 rounded-3xl p-6">
                  <div className="flex flex-col gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 flex justify-between items-end">
                        <span>Height</span>{" "}
                        <span className="text-slate-300 text-[9px] lowercase opacity-100">
                          in cm
                        </span>
                      </label>
                      <input
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-100">
                    ...
                    <button
                      ...
                      className="w-full mt-3 flex items-center justify-center gap-1 text-xs font-semibold text-slate-400 hover:text-slate-700 transition-colors py-1"
                    >
          ...
          {step === "scanning" && (
            ...
            >
              <div className="relative w-32 h-32 mb-8">
                ...
                />
                <div
                  className="absolute inset-3 rounded-full border-[3px] border-transparent animate-spin"
                  style={{
              ...
              <motion.p
                className="text-slate-500 text-sm mb-8 h-4 font-mono uppercase tracking-widest text-[10px]"
                key={scanPhase}
              ...
              <div className="w-full max-w-sm space-y-2.5">
                <div className="h-1.5 rounded-full overflow-hidden bg-slate-200">
                  <motion.div
                    ...
                    style={{
                      background: `linear-gradient(90deg, ${accent}, #3730A3)`,
                    }}
          ...
          {step === "results" && result && (
            ...
            >
              ...
              <div className="flex flex-col lg:flex-row gap-6">
                {/* 3D Model Area */}
                <div
                  className="lg:w-[340px] shrink-0 rounded-3xl overflow-hidden border border-slate-200 relative bg-slate-50 shadow-inner"
                  style={{ height: 480 }}
                >
                  <div className="absolute top-4 left-4 z-10">
                    <span className="px-3 py-1.5 rounded-xl text-[10px] font-bold text-slate-600 uppercase tracking-widest border border-slate-200 bg-white/80 backdrop-blur-md shadow-sm">
                      Interactive 3D
                    </span>
                  </div>
                  {result.obj ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full gap-3 text-slate-400">
                      ...
                      <p className="text-xs">3D mesh failed to generate</p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex-1 flex flex-col gap-4">
                  <div className="rounded-3xl border border-slate-200 p-6 bg-white shadow-xl shadow-slate-200/50">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {MEASUREMENT_ORDER.filter(
                        (k) => result.measurements?.[k] != null,
                      ).map((key, i) => {
