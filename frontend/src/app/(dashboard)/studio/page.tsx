...
const GarmentVector = ({ type, color }: { type: string; color: string }) => {
  const baseClass =
    "w-full h-full drop-shadow-2xl transition-all duration-300 pointer-events-none";
  const defs = (
    <defs>
  ...
  if (type === "tshirt") {
    return (
      ...
      >
        ...
        <g fill="url(#contour)">
          <path
            d="M 30 15 Q 50 22 70 15 L 75 85 C 75 90 25 90 25 85 Z"
            {...fillProps}
          />
          <path
            d="M 30 15 L 12 32 C 8 36 18 45 23 40 L 28 35 Z"
            {...fillProps}
          />
          <path
            d="M 70 15 L 88 32 C 92 36 82 45 77 40 L 72 35 Z"
            {...fillProps}
          />
          <path
            d="M 40 15 Q 50 24 60 15"
            fill="none"
  ...
  if (type === "hoodie") {
    return (
      ...
      >
        ...
        <g fill="url(#contour)">
          <path
            d="M 30 25 Q 50 32 70 25 L 75 85 C 75 90 25 90 25 85 Z"
            {...fillProps}
          />
          <path
            d="M 30 25 L 8 50 C 5 55 15 60 20 55 L 25 40 Z"
            {...fillProps}
          />
          <path
            d="M 70 25 L 92 50 C 95 55 85 60 80 55 L 75 40 Z"
            {...fillProps}
          />
          <path
            d="M 37 25 C 34 -3 66 -3 63 25 Z"
            {...fillProps}
            strokeWidth="1"
          />
  ...
  if (type === "dress") {
    return (
      ...
      >
        ...
        <g fill="url(#contour)">
          <path
            d="M 40 10 L 42 25 L 58 25 L 60 10"
            fill="none"
            stroke={color}
            strokeWidth="2.5"
          />
          <path
            d="M 33 25 L 67 25 L 72 45 L 88 95 C 88 98 12 98 12 95 L 28 45 Z"
            {...fillProps}
          />
          <path
            d="M 35 45 Q 50 50 65 45"
            fill="none"
            stroke="rgba(0,0,0,0.1)"
            strokeWidth="1"
          />
  ...
  if (type === "jacket") {
    return (
      ...
      >
        ...
        <g fill="url(#contour)">
          <path
            d="M 28 15 Q 50 20 72 15 L 78 85 C 78 90 22 90 22 85 Z"
            {...fillProps}
          />
          <path
            d="M 28 15 L 8 45 C 5 50 15 55 20 50 L 25 35 Z"
            {...fillProps}
          />
          <path
            d="M 72 15 L 92 45 C 95 50 85 55 80 50 L 75 35 Z"
            {...fillProps}
          />
          <path
            d="M 50 18 L 50 90"
  ...
  if (type === "polo") {
    return (
      ...
      >
        ...
        <g fill="url(#contour)">
          <path
            d="M 28 20 Q 50 25 72 20 L 75 85 C 75 90 25 90 25 85 Z"
            {...fillProps}
          />
          <path
            d="M 28 20 L 12 35 C 8 40 18 45 23 40 L 28 32 Z"
            {...fillProps}
          />
          <path
            d="M 72 20 L 88 35 C 92 40 82 45 77 40 L 72 32 Z"
            {...fillProps}
          />
...
export default function StudioPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "garment" | "text" | "paint" | "graphics" | "layers"
  >("garment");
  ...
  const [history, setHistory] = useState<HistoryState[]>([
    { elements: [], frontPaintData: "", backPaintData: "" },
  ]);
  ...
  const handleUndo = () => {
    if (historyIndex > 0) {
      ...
      setFrontPaintData(p.frontPaintData);
      setBackPaintData(p.backPaintData);
  ...
  const syncPaint = useCallback(() => {
    let f = frontPaintData,
      b = backPaintData;
    if (paintCanvasFrontRef.current)
      f = paintCanvasFrontRef.current.toDataURL();
  ...
  const paint = (e: React.PointerEvent<HTMLCanvasElement>) => {
    const pc =
      currentSide === "front"
        ? paintCanvasFrontRef.current
        : paintCanvasBackRef.current;
    if (!pc) return;
  ...
  useEffect(() => {
    const handleMove = (e: PointerEvent) => {
      if (!isDraggingObj || !dragState.current || !elementsAreaRef.current)
        return;
      e.preventDefault();
      ...
      setElements((prev) =>
        prev.map((p) =>
          p?.id === dragState.current?.id ? { ...p, x: newX, y: newY } : p,
        ),
      );
    };
  ...
  const handleSave = async () => {
    ...
    if (artboardRef.current) {
      try {
        ...
        const uploadResult = await uploadSnapshotBase64(
          compositeBase64,
          timestampId,
        );
        ...
        setHistory([
          {
            ...
            frontPaintData: config.frontPaintData || "",
            backPaintData: config.backPaintData || "",
          },
        ]);
        setHistoryIndex(0);
        return;
    ...
    <div className="flex flex-col h-screen bg-[#F0F0F0] font-sans selection:bg-black selection:text-white">
      ...
      <div className="flex items-center justify-between px-6 py-2 border-b border-gray-200 z-40 shrink-0 bg-white h-[60px] shadow-sm">
        <div className="flex items-center gap-4">
          <Link
            href="/dashboard"
            className="flex items-center gap-1.5 text-gray-400 hover:text-black transition-colors text-xs font-black uppercase tracking-widest"
          >
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black tracking-widest uppercase text-white hover:scale-105 active:scale-95 transition-all shadow-md"
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className="studio-dark-panel w-[320px] shrink-0 bg-white border-r border-gray-200 flex flex-col overflow-hidden shadow-2xl shadow-indigo-900/10 z-20">
          <div className="flex bg-[#F8F8F8] p-1.5 border-b border-gray-200">
            <button
              ...
              className={`flex-1 py-2 text-[10px] font-black uppercase tracking-[0.2em] rounded-md transition-all ${currentSide === "front" ? "bg-white shadow-sm text-black" : "text-gray-400 hover:text-gray-700"}`}
            >
              Front View
            </button>
            <button
              onClick={() => setCurrentSide("back")}
              className={`flex-1 py-2 text-[10px] font-black uppercase tracking-[0.2em] rounded-md transition-all ${currentSide === "back" ? "bg-white shadow-sm text-black" : "text-gray-400 hover:text-gray-700"}`}
            >
              Back View
            </button>
          </div>
          ...
          <div className="flex border-b border-gray-100">
            {[
              { id: "garment", label: "Style", Icon: Shirt },
              { id: "paint", label: "Draw", Icon: Paintbrush },
              { id: "text", label: "Type", Icon: Type },
              { id: "graphics", label: "Assets", Icon: ImageIcon },
            ].map((t) => (
              <button
                key={t.id}
                onClick={() => setActiveTab(t.id as any)}
                className="flex-1 py-4 flex flex-col items-center gap-1.5 text-[9px] font-black uppercase tracking-wider transition-all hover:bg-gray-50"
                style={{
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-6">
            {/* GARMENT */}
            <div
              className={activeTab === "garment" ? "block space-y-6" : "hidden"}
            >
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {GARMENT_TYPES.map((g) => (
                    <button
                      ...
                      className="flex items-center justify-between px-3 py-3 rounded-xl border-2 transition-all hover:border-gray-400 bg-white"
                      style={{
                        borderColor:
                          garmentType === g.id ? "#1A1A1A" : "#F0F0F0",
                        transform:
                          garmentType === g.id ? "scale(1.02)" : "scale(1)",
                      }}
                    >
                ...
                <div className="grid grid-cols-5 gap-2">
                  {COLORS.map((c) => (
                    <button
                      ...
                      className="w-full aspect-square rounded-full border border-black/10 transition-transform hover:scale-110 shadow-sm"
                      style={{
                        background: c,
                        outline:
                          garmentColor === c ? "2px solid #1A1A1A" : "none",
                        outlineOffset: "2px",
                      }}
                    />
                  ))}
            ...
            {/* PAINT */}
            <div
              className={activeTab === "paint" ? "block space-y-4" : "hidden"}
            >
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-2">
                Interactive Canvas ({currentSide})
              </p>
              <div className="aspect-square w-full rounded-2xl bg-white border-2 border-dashed border-gray-200 overflow-hidden relative shadow-sm cursor-crosshair">
                ...
                />
                <canvas
                  ref={paintCanvasFrontRef}
                  width={800}
                  height={800}
                  className={`w-full h-full touch-none relative z-10 ${currentSide === "front" ? "block" : "hidden"}`}
                ...
                />
                <canvas
                  ref={paintCanvasBackRef}
                  width={800}
                  height={800}
                  className={`w-full h-full touch-none relative z-10 ${currentSide === "back" ? "block" : "hidden"}`}
                  onPointerDown={handlePaintStart}
                  onPointerMove={handlePaintMove}
                  onPointerUp={handlePaintEnd}
                  onPointerLeave={handlePaintEnd}
              ...
              <div className="grid grid-cols-2 gap-2 mt-2">
                <button
                  ...
                  className={`py-3 rounded-xl border-2 text-[11px] font-black uppercase tracking-wider flex items-center justify-center gap-2 ${!isEraser ? "bg-[#1A1A1A] text-white border-black" : "bg-white text-gray-500 hover:bg-gray-50 border-gray-200"}`}
                >
              ...
              <div className="flex gap-3 p-1.5 border border-gray-100 rounded-2xl bg-white shadow-sm">
                <input
                  ...
                  className="w-12 h-12 rounded-xl cursor-pointer border border-gray-200 shadow-inner"
                />
                <div className="flex-1 px-3 flex flex-col justify-center">
                  <span className="text-[9px] font-black uppercase text-gray-400 tracking-widest -mt-1">
                    Line Width
                  </span>
                  <input
                    type="range"
                    ...
                    className="w-full accent-[#1A1A1A] mt-1"
                  />
                </div>
              </div>
              <button
                onClick={clearPaintCanvas}
                className="w-full py-3 mt-1 rounded-xl border-2 border-transparent text-[11px] font-black uppercase tracking-wider text-red-500 flex justify-center items-center gap-2 hover:bg-red-50 hover:border-red-100 transition-colors"
              >
                <RotateCcw className="w-4 h-4" /> Clear Layer
              </button>
            </div>

            {/* TEXT */}
            <div
              className={activeTab === "text" ? "block space-y-4" : "hidden"}
            >
              <textarea
                value={textContent}
                onChange={(e) => setTextContent(e.target.value)}
                className="w-full bg-white border-2 border-gray-200 rounded-xl px-4 py-3 text-sm font-bold text-gray-800 outline-none resize-none focus:border-black transition-all shadow-sm"
                rows={2}
                placeholder="Add typography..."
              />
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  className="flex-1 border-2 border-gray-200 rounded-xl px-3 py-3 text-xs font-black uppercase tracking-wider outline-none bg-white"
                >
                  {FONTS.map((f) => (
                ...
                <input
                  ...
                  className="w-12 h-12 rounded-xl cursor-pointer border border-gray-200 shadow-sm"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`py-3 rounded-xl border-2 font-black uppercase tracking-wider text-xs ${textBold ? "bg-[#1A1A1A] text-white border-black" : "bg-white text-gray-500 hover:bg-gray-50 border-gray-200"}`}
                >
                  Bold
                </button>
                <button
                  onClick={() => setTextItalic((b) => !b)}
                  className={`py-3 rounded-xl border-2 italic font-bold uppercase tracking-wider text-xs ${textItalic ? "bg-[#1A1A1A] text-white border-black" : "bg-white text-gray-500 hover:bg-gray-50 border-gray-200"}`}
                >
                  Italic
                </button>
              </div>
              ...
              <button
                onClick={addTextEl}
                className="w-full py-4 mt-2 rounded-xl font-black uppercase tracking-widest text-sm text-white flex items-center justify-center bg-black hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl"
              >
            ...
            <div
              className={
                activeTab === "graphics" ? "block space-y-4" : "hidden"
              }
            >
              <div className="flex gap-2">
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full border-2 border-gray-200 rounded-xl pl-4 pr-10 py-3 text-xs font-bold outline-none focus:border-black transition-colors"
                    placeholder="Search patterns or vectors..."
                  />
                  {unsplashLoading && (
                ...
                <button
                  ...
                  className="px-5 rounded-xl bg-black text-white hover:scale-105 active:scale-95 transition-all"
                >
              ...
              <div className="grid grid-cols-2 gap-3 max-h-96 overflow-y-auto pr-1">
                {unsplashResults.map((img) =>
                  img ? (
                    <button
                      key={img.id}
                      onClick={() => addUnsplashImage(img.urls?.regular)}
                      className="rounded-xl overflow-hidden aspect-square border-2 border-transparent hover:border-black transition-all shadow-sm bg-gray-100 group relative"
                    >
                      <img
                        src={img.urls?.thumb}
                ...
                {unsplashResults.length === 0 && !unsplashLoading && (
                  <div className="col-span-2 py-10 flex flex-col items-center justify-center text-gray-300 border-2 border-dashed border-gray-200 rounded-xl">
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at center, #FFFFFF 0%, #E5E5E5 100%)",
          }}
        >
          ...
          <div
            ref={artboardRef}
            className="relative w-full max-w-[550px] aspect-square flex items-center justify-center bg-transparent"
          >
            {/* Base Garment Vector */}
            ...
            <div
              ref={elementsAreaRef}
              className="absolute inset-[10%] z-20 overflow-hidden"
              style={{
                clipPath: garmentType === "tshirt" ? "inset(0)" : "none",
              }}
            >
              {/* Sync'd Paint Layer */}
              {currentSide === "front" && frontPaintData && (
                <img
                  src={frontPaintData}
                  className="absolute inset-0 w-full h-full opacity-90 mix-blend-multiply pointer-events-none"
                  alt=""
                />
              )}
              {currentSide === "back" && backPaintData && (
                <img
                  src={backPaintData}
                  className="absolute inset-0 w-full h-full opacity-90 mix-blend-multiply pointer-events-none"
                  alt=""
                  ...
                  el ? (
                    ...
                    >
                      {el.type === "image" ? (
                        <img
                          src={el.content}
                          className="w-full h-auto drop-shadow-xl mix-blend-multiply rounded-lg select-none pointer-events-none"
                          draggable={false}
                          alt=""
                        />
                      ) : (
        ...
        <div className="studio-dark-panel w-[300px] shrink-0 bg-white border-l border-gray-200 flex flex-col overflow-hidden shadow-2xl shadow-indigo-900/10 z-20">
          <div className="flex border-b border-gray-100">
            {[
              { id: "layers", label: "Object Layers", Icon: Layers },
              { id: "mydesigns", label: "Saved Works", Icon: FolderOpen },
            ].map((t) => (
              <button
                key={t.id}
                onClick={() =>
                  t.id === "mydesigns" ? openMyDesigns() : setRightTab("layers")
                }
                ...
                style={{
                  color: rightTab === t.id ? "#1A1A1A" : "#BBB",
                  borderBottom:
                    rightTab === t.id
                      ? "3px solid #1A1A1A"
                      : "3px solid transparent",
                  background: rightTab === t.id ? "#FAFAFA" : "#fff",
                }}
              >
              ...
              <>
                {["front", "back"].map((side) => {
                  ...
                  return (
                    <div key={side} className="mb-6">
                      <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3 pl-1">
                        {side} Decals
                      </p>
                      <div className="space-y-3">
                        {sideEls.map((el, i) =>
                          el ? (
                            <div
                              key={el.id}
                              className="p-3 border-2 border-gray-100 rounded-xl bg-white flex flex-col gap-3 shadow-sm group hover:border-gray-300 transition-all"
                            >
                              <div className="flex items-center gap-3">
                                <span className="text-[9px] font-black bg-gray-100 text-gray-500 w-5 h-5 flex items-center justify-center rounded-md uppercase shrink-0">
                                  {i + 1}
                                </span>
                                <span className="text-[11px] font-black text-gray-800 truncate flex-1 uppercase tracking-wider">
                                  {el.type === "image"
                                    ? "Art Asset"
                                    : el.content}
                                </span>
                              ...
                              <div className="flex flex-col gap-2 bg-[#FAFAFA] p-2.5 rounded-lg border border-gray-100">
                                <div className="flex items-center gap-3">
                                  <span className="text-[9px] font-black text-gray-400 uppercase w-6">
                                    SCL
                                  </span>
                                  <input
                                    type="range"
                ...
                {elements.length === 0 && (
                  <div className="text-center py-12 text-gray-400 border-2 border-dashed border-gray-200 rounded-2xl bg-white">
                    <Layers className="w-10 h-10 mx-auto mb-3 opacity-20 text-gray-900" />
                ...
                {loadingDesigns ? (
                  ...
                  myDesigns.map((d) =>
                    d ? (
                      <div
                        key={d.id}
                        className="border-2 border-transparent bg-white p-4 rounded-xl hover:border-black cursor-pointer transition-all shadow-sm group"
                        onClick={() => loadDesign(d)}
                      >
                        <div className="flex justify-between items-start mb-2">
                          <p className="text-[13px] font-black text-gray-900 truncate tracking-tight">
                            {d.title || (d as any).name}
                          </p>
                          <div
                            className="w-5 h-5 rounded-full shrink-0 border-2 border-white shadow-md outline outline-2 outline-gray-200"
                            style={{
                              background: (d as any).garment_color || "#1A1A1A",
                            }}
                          />
                        </div>
                        <p className="text-[9px] text-gray-400 font-black uppercase tracking-[0.2em]">
                          {d.type || (d as any).garment_type}
                        </p>
                        {d.storage_url && (
                          <img
                            src={d.storage_url}
                            className="w-full h-24 object-contain rounded-md mt-2 opacity-60 bg-gray-50/50"
                          />
                        )}
                      </div>
                    ) : null,
